      ...
      <td>
        <Box>
          <Stack spacing={2} direction="row">
            ...
            <IconButton variant="contained" size="sm" underline="hover" onClick={ () => window.open(`/activities/${activity.id}`) }>
              <DoneIcon /></IconButton>
            </Tooltip>
            
            <Tooltip title="Delete Activity">
              <IconButton size="sm" variant="outlined" underline="hover" onClick={
                () => JsonData.splice(activity.id, 1)}>
                <DeleteIcon /></IconButton> 
            </Tooltip>
             <Tooltip title="Subscribe to Activity">
             <IconButton variant="contained" size="small" underline="hover" onClick={ () => window.open(`/subscribe/${activity.id}`) }>
